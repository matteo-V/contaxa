...
###################################################################################
###### function for calculating percent by gender ##################################
# function takes a joined taxa/demographic data set and demo data set
# and calculates percentage of respondents reporting each contact/taxa type combo
#'@param dat dataset generated by reshape_contaxa data function
#'@param demo_dat dataset generated by the select_demo_dat function
#'@return dataset with percentage of respondents with contacts as percent
#'@author matteo-V
calc_percent_respondents_by_gender <- function(dat, demo_dat){
  dat %>%
    #group by gender, contact and taxa types
    group_by(gender,
             contx_type,
             taxa_type) %>%
    summarise(num_respondents = n() ) %>% #count number of contact/taxa types by gender
    mutate_at(
      .vars = 'num_respondents',
      .funs = funs(
        case_when(
          gender == 'male' ~ . / calc_total_respondents(demo_dat, var_name = 'gender', var_val = 'male'),
          gender == 'female' ~ . / calc_total_respondents(demo_dat, var_name = 'gender', var_val = 'female')
        )
      )
    ) %>%
    #rename var to reflect change to percentage
    rename(percent_respondents = num_respondents)

}
##################################################################################################
#################################calc respondents by age function ################################
#TODO: make this more general, maybe use a loop for all quintiles?
#TODO: this will make the function more general and can refactor the age quintile function
# calculate percentage of respondents by age quintile
# data can be generated using get_age_quintiles
calc_percent_respondents_by_age <- function(dat, demo_dat){
  dat %>%
    group_by(age_quint,
             contx_type,
             taxa_type) %>%
    ...
    mutate_at(.vars = 'num_respondents',
              .funs = funs(
                #can this data set be created iteratively then joined at the end?
                case_when(
                  age_quint == 1 ~ . / calc_total_respondents(demo_dat, var_name ='age_quint', var_val = 1),
                  age_quint == 2 ~ . / calc_total_respondents(demo_dat, var_name ='age_quint', var_val = 2),
                  age_quint == 3 ~ . / calc_total_respondents(demo_dat, var_name ='age_quint', var_val = 3),
                  age_quint == 4 ~ . / calc_total_respondents(demo_dat, var_name ='age_quint', var_val = 4),
                  age_quint == 5 ~ . / calc_total_respondents(demo_dat, var_name ='age_quint', var_val = 5)
                )
              )
    ) %>%
    rename(percent_respondents = num_respondents)
...
#################################################################################################
#################################### cal respondents by live function ###########################
#TODO: play with this function and see if you can make it general
#TODO: perhaps require passing in a vector of  site names or can parse from passed data set?
#TODO: can wrap this into a for loop through the values of the variable and use mutate with if_else
calc_percent_repondents_by_live <- function(dat, demo_dat){
  #pipeline for BD sites
  dat %>%
    group_by(live_state_prov,
             contx_type,
             taxa_type) %>%
    summarise(num_respondents = n()) %>%
    mutate_at(.vars = 'num_respondents',
              .funs = funs(
                case_when(
                  live_state_prov == 'Rangpur Division' ~ . / calc_total_respondents(demo_dat, var_name = 'live_state_prov', var_val = 'Rangpur Division'),
                  live_state_prov == 'Dhaka' ~ . / calc_total_respondents(demo_dat, var_name = 'live_state_prov', var_val = 'Dhaka'),
                  live_state_prov == 'Jhinaidhaha' ~ . / calc_total_respondents(demo_dat, var_name = 'live_state_prov', var_val = 'Jhinaidhaha'),
                  live_state_prov == 'Khulna' ~ . / calc_total_respondents(demo_dat, var_name = 'live_state_prov', var_val = 'Khulna')
                )
              )
            ) %>%
    rename(percent_respondents = num_respondents)
...
##################################################################################################
######################### cal respondents by education ###########################################

calc_percent_repondents_by_education <- function(dat, demo_dat){
  #pipeline for BD sites
  dat %>%
    ...
    mutate_at(.vars = 'num_respondents',
              .funs = funs(
                case_when(
                  highest_education == 'college/university/professional' ~ . / calc_total_respondents(demo_dat, var_name = 'highest_education', var_val = 'college/university/professional'),
                  highest_education == 'none' ~ . / calc_total_respondents(demo_dat, var_name = 'highest_education', var_val = 'none'),
                  highest_education == 'primary school' ~ . / calc_total_respondents(demo_dat, var_name = 'highest_education', var_val = 'primary school'),
                  highest_education == 'secondary school' ~ . / calc_total_respondents(demo_dat, var_name = 'highest_education', var_val = 'secondary school')
                )
              )
    ) %>%
    rename(percent_respondents = num_respondents)
...
##################################################################################################
####################### cal respondents by maternal education ####################################

calc_percent_repondents_by_education_mother <- function(dat, demo_dat){
  #pipeline for BD sites
  dat %>%
    group_by(highest_education_mother,
             contx_type,
             taxa_type) %>%
    summarise(num_respondents = n()) %>%
    mutate_at(.vars = 'num_respondents',
              .funs = funs(
                case_when(
                  highest_education_mother == 'college/university/professional' ~ . / calc_total_respondents(demo_dat, var_name = 'highest_education_mother', var_val = 'college/university/professional'),
                  highest_education_mother == 'none' ~ . / calc_total_respondents(demo_dat, var_name = 'highest_education_mother', var_val = 'none'),
                  highest_education_mother == 'primary school' ~ . / calc_total_respondents(demo_dat, var_name = 'highest_education_mother', var_val = 'primary school'),
                  highest_education_mother == 'secondary school' ~ . / calc_total_respondents(demo_dat, var_name = 'highest_education_mother', var_val = 'secondary school')
                )
              )
    ) %>%
    rename(percent_respondents = num_respondents)
...
###################################################################################################
##### test age and gender grouping functions ###############
# #gender group
# tst_group_gender_fn <-
#   tst_ntile_joined_dat %>%
#   calc_percent_respondents_by_gender(demo_dat = tst_demo)
#
# #age group
# tst_group_age_fn <-
#   tst_ntile_joined_dat %>%
#   calc_percent_respondents_by_age(demo_dat = tst_demo)
#
# #live group
# tst_group_live_fn <-
#   tst_ntile_joined_dat %>%
#   calc_percent_repondents_by_live(demo_dat = tst_demo)
#
# #education group
# tst_group_education <-
#   tst_ntile_joined_dat %>%
#   calc_percent_repondents_by_education(demo_dat = tst_demo)
#
# #maternal education group
# tst_group_education_mother <-
#   tst_ntile_joined_dat %>%
#   calc_percent_repondents_by_education_mother(demo_dat = tst_demo)
...
#'@author matteo-V
#'@depends get_var_vals
#'@function to fill missing rows in gender grouped, aggregated dataframe
expand_contaxa_by_gender <- function(dat){
  #create grid expansion for filling missing crossproducts
  support <- expand.grid(gender = dat %>% get_var_vals(var_name = 'gender'),
                         contx_type = dat %>% get_var_vals(var_name = 'contx_type'), #TODO: replace with all contx_type
                         taxa_type = dat %>% get_var_vals(var_name = 'taxa_type'),
                         stringsAsFactors = FALSE
  )
  #fill missing crossprods gender X contx_type X taxa_type
  dat %>%
    replyr_coalesce(support = support,
                    fills= list(percent_respondents = 0.0)) # fill missing with 0.00
}
#####################################################################################
################ expand crossprod by age #########################################
#TODO: DOCS
#TODO: can we make this more general? again passing in variable by symbol?
expand_contaxa_by_age <- function(dat){
  #TODO: we can make this general by passing a string with the variable to group by
  support <- expand.grid(age_quint = dat %>% get_var_vals(var_name = 'age_quint'),
                         contx_type = dat %>% get_var_vals(var_name = 'contx_type'), #TODO: replace with all contx types
                         taxa_type = dat %>% get_var_vals(var_name = 'taxa_type'),
                         stringsAsFactors = FALSE)
  #fill missing crossprods gender X contx_type X taxa_type
  dat %>%
...
######################################################################################
###########################expand crossprod by live ##################################

expand_contaxa_by_live <- function(dat){
  #TODO: we can make this general by passing a string with the variable to group by
  support <- expand.grid(live_state_prov = dat %>% get_var_vals(var_name = 'live_state_prov'),
                         contx_type = dat %>% get_var_vals(var_name = 'contx_type'), #TODO: replace with all contx types
                         taxa_type = dat %>% get_var_vals(var_name = 'taxa_type'),
                         stringsAsFactors = FALSE)
  #fill missing crossprods gender X contx_type X taxa_type
...
######################################################################################
################################### expand crossprod by education ####################

expand_contaxa_by_education <- function(dat){
  #TODO: we can make this general by passing a string with the variable to group by
  support <- expand.grid(highest_education = dat %>% get_var_vals(var_name = 'highest_education'),
                         contx_type = dat %>% get_var_vals(var_name = 'contx_type'),
                         taxa_type = dat %>% get_var_vals(var_name = 'taxa_type'),
                         stringsAsFactors = FALSE)
  #fill missing crossprods gender X contx_type X taxa_type
  dat %>%
...
######################################################################################
############################## expand crossprod by maternal education ################

expand_contaxa_by_education_mother <- function(dat){
  #TODO: we can make this general by passing a string with the variable to group by
  support <- expand.grid(highest_education_mother = dat %>% get_var_vals(var_name = 'highest_education_mother'),
                         contx_type = dat %>% get_var_vals(var_name = 'contx_type'),
                         taxa_type = dat %>% get_var_vals(var_name = 'taxa_type'),
                         stringsAsFactors = FALSE)
  #fill missing crossprods gender X contx_type X taxa_type
...
########################################################################################
################# output gender cross prod expansion function ##########################
...
BD_age_annotated <-
  ...
  annotate_factor_with_n(var_name = 'age_quint_range',
                         demo_dat = BD_demo)
# #save BD gender data for testing viz functions
# saveRDS(tst_group_gender_fill, file='data/BD_contaxa_percent_by_gender.rds')
#
# #test age crossprod expansion function
# tst_group_age_fill <-
#   tst_group_age_fn %>%
#   expand_contaxa_by_age()
#
# #save BD age data for testing viz functions
# saveRDS(tst_group_age_fill, file='data/BD_contaxa_percent_by_age.rds')
#
# #test age crossprod
# tst_group_live_fill <-
#   tst_group_live_fn %>%
#   expand_contaxa_by_live()
#
# #save BD live data
# saveRDS(tst_group_live_fill, file='data/BD_contaxa_percent_by_live.rds')
#
# #test education crossprod
# tst_group_education_fill <-
#   tst_group_education %>%
#   expand_contaxa_by_education()
#
# #save BD education data
# saveRDS(tst_group_education_fill, file='data/BD_contaxa_percent_by_education.rds')
#
#
# #test maternal education crossprod
# tst_group_education_mother_fill <-
#   tst_group_education_mother %>%
#   expand_contaxa_by_education_mother()
#
# #save BD education data
# saveRDS(tst_group_education_mother_fill, file='data/BD_contaxa_percent_by_education_mother.rds')
